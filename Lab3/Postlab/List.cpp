...
ListItr List::first(){
  ListItr retVal;
  retVal.current = head->next; //element after head ListNode
  return retVal; 
}
...
void List::insertBefore(int x, ListItr position){
  ...
  position.current->previous->next = inside; //currents previous points to inserted node 
  position.current->previous = inside; 
